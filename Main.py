...
selectedFeature = "polygonApproximation"
# Extract the featureVectors for all the writers may need additional preprocessing inside.
classifiedCO3,featureVectors=featureExtraction.extractFeatures(selectedFeature, dataSet.trainingDataImages)
...
if constants.identification=="svm":
    ...
    performanceEvaluation.evaluatePerformanceSVM(svclassifier,selectedFeature,dataSet.testingDataImages,dataSet.testingDataWriters,classifiedCO3)
else:
    performanceEvaluation.evaluatePerformance(selectedFeature,dataSet.testingDataImages,dataSet.testingDataWriters,featureVectors,dataSet.tariningDataWriters,classifiedCO3)
...
if constants.identification=="svm":
    writerId=SVMIdentification.identifyWriterSVM(svclassifier,selectedFeature,writerImages[0],classifiedCO3)
else:
